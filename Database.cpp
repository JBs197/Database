...
int download(wstring url, wstring folder, wstring filename)
{
	...
	HANDLE hprinter = CreateFileW(filepath.c_str(), (GENERIC_READ | GENERIC_WRITE), (FILE_SHARE_READ | FILE_SHARE_WRITE), NULL, CREATE_NEW, 0, NULL);
	...
	if (!CloseHandle(hprinter)) { warn(L"CloseHandle-download"); }
...
int CSV_DB::read()
{
	...
	while (1) // For every line in the csv file...
	{
		...
		else if (temp_columns[0] == L"Note") { break; }
		else
		{
...
int main()
{
	...
	vector<vector<CATA_DB>> catalogue_tree = initialize(database, local_directory);
	/*
	vector<wstring> subfolders; 
	sqlite3* database = createDB(year, subfolders);
	for (int ii = 0; ii < subfolders.size(); ii++) 
	{
		tablemaker(database, subfolders[ii]);
	}
	system("pause");
	*/
	return 0;
